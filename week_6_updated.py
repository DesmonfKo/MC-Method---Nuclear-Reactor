# -*- coding: utf-8 -*-
"""
Created on Tue Mar 11 12:07:49 2025
...
def validate_moderator(input_str):
    return input_str.lower() in ['water', 'heavywater', 'graphite']  # Case-insensitive
...
class ReactorMixture:
    ...
    @property
    def macroscopic_capture(self):
        return self.macroscopic_absorption - self.macroscopic_fission
...
def simulate_generation(neutron_positions, simulator, generation_number):
    ...
    for pos in neutron_positions:
        if generation_number > 1:
            ...
            if not is_inside_reactor(pos, simulator.geometry, simulator.size):
                leaked += 1
                continue
        r = np.random.rand()
        if r < P_scatter:
    ...
    stats = {
        ...
        'n_leaked': leaked,
        'n_absorbed': len(absorbed_positions),
    }
    return new_neutron_positions, stats
def is_inside_reactor(position, geometry, size):
    if geometry == 'sphere':
        return np.linalg.norm(position) <= size
    elif geometry == 'cylinder':
        r = np.sqrt(position[0]**2 + position[1]**2)
        return (r <= size[0]) and (abs(position[2]) <= size[1]/2)
    else:
        return False
def compute_k_factor(offspring_counts, original_count, p_fission=0.1, nu=2.4):
    """
    Uncertainty based on:
    σ² = n * [p_fission * ν(1 + ν) + (1-p_fission)*0] 
    σ_k = sqrt(σ²)/n
    """
    if original_count == 0:
        return 0, 0
    
    k = sum(offspring_counts) / original_count
    variance = original_count * p_fission * nu * (1 + nu)
    uncertainty = np.sqrt(variance) / original_count
...
# ----------------------------------------------------
def plot_generation_histograms_volume_normalized_sphere(stats, reactor_radius, N_initial, generation):
    """
    ...
    plt.title(f'Generation {generation} Collision Distribution (Volume Normalized)\n'
...
# ----------------------------------------------------
def plot_generation_histograms_volume_normalized_cylinder(stats, radius, height, N_initial, generation):
    """
    Plot a volume-normalized histogram of collisions from the first generation
        ...
        for R_mtf in R_mtfs:
            for u235_pct in U235_percents:
                if geometry == 'cylinder':
                    ...
                    _, stats = simulate_generation(neutrons, sim_temp, generation_number=1)
                    k, _ = compute_k_factor(stats['offspring_counts'], initial_neutron_count)
                    if abs(k - 1) < tolerance:
                        matching_sets.append({
                            'radius (m)': r,
                            'R_mtf': R_mtf,
...
def extended_main():
    ...
    moderator = get_valid_input(
        "Moderator [water/heavywater/graphite]: ",  # Lowercase options
        validate_moderator,
        "Must be water, heavywater, or graphite"
    ).lower()  # Store as lowercase
    ...
    R_mtf = float(get_valid_input(
        ...
        lambda x: validate_float(x, 0),  # Allow 0
        "Must be ≥ 0"
    ))
    ...
    print(f"Σs: {simulator.mixture.macroscopic_scattering:.2e} 1/m")
    print(f"Σa: {simulator.mixture.macroscopic_absorption:.2e} 1/m")
    print(f"Σf: {simulator.mixture.macroscopic_fission:.2e} 1/m")
    print(f"Σc (capture): {simulator.mixture.macroscopic_capture:.2e} 1/m")
    ...
    for g in range(2, Q+1):
        if len(neutrons) == 0:
            ...
            break
        original_count = len(neutrons)
        neutrons, stats = simulate_generation(neutrons, simulator, g)
        p_fission = simulator.fission_probability()
        nu = simulator.mixture.average_neutrons_per_fission
        k, unc = compute_k_factor(stats['offspring_counts'], original_count, p_fission, nu)
        k_factors.append(k)
        uncertainties.append(unc)
        generation_stats.append(stats)
        print(f"Generation {g}: k-factor = {k:.3f} ± {unc:.3f} | "
      f"Absorbed: {stats['n_absorbed']} | Leaked: {stats['n_leaked']}")

    # Plot first-generation collisions
    if geometry == 'sphere':
        # Volume-normalized histogram (spherical)
        plot_generation_histograms_volume_normalized_sphere(
            generation_stats[0], size, N_initial, 0
        )
        plot_generation_histograms_volume_normalized_sphere(
            generation_stats[4], size, N_initial, 4
        )
        plot_generation_histograms_volume_normalized_sphere(
            generation_stats[9], size, N_initial, 9
        )
    else:
        # Volume-normalized histogram (cylindrical)
        plot_generation_histograms_volume_normalized_cylinder(
            generation_stats[0], size[0], size[1], N_initial, 0
        )
        # Volume-normalized histogram (cylindrical)
        plot_generation_histograms_volume_normalized_cylinder(
            generation_stats[4], size[0], size[1], N_initial, 4
        )
        # Volume-normalized histogram (cylindrical)
        plot_generation_histograms_volume_normalized_cylinder(
            generation_stats[9], size[0], size[1], N_initial, 9
        )
