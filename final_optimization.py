...
def simulate_first_generation(mixture, geometry, size, N0,
    ...
    if geometry=='sphere':
        ...
        R_refl= R_core+ reflector_thickness
        H_refl= H_core+ 2.*reflect_thickness
...
def compute_k_factor_and_uncertainty(n_abs, n_init, fission_prob, nu):
    """
    Return (k, dk) => consistent naming => fix the NameError by using dk_ consistently.
    """
    if n_init<=0:
        return (0.0, 0.0)
    k_ = (n_abs/n_init)* fission_prob* nu
    if (n_abs>0) and (0<= fission_prob<=1):
        stdev_f= np.sqrt(n_abs*fission_prob*(1.- fission_prob))
        dk_= (nu/n_init)* stdev_f
    else:
        dk_= 0.0
    return (k_, dk_)
...
def final_bounding_box_sweep_multi_gen(geometry, size, 
    ...
    for i_f, ff in enumerate(f_vals):
        for j_r, rr in enumerate(r_vals):
            ...
            (k1, dk1)= compute_k_factor_and_uncertainty(ab1, n_neut_sweep,
            ...
            if abs(k1-1.)<0.1:
                ...
                for g_ in range(2, max_generations+1):
                    ...
                    if nab<100:
                        if nab==0:
                            break
                        idx= np.random.choice(nab, size=100, replace=True)
                        pos= g_res['absorbed_positions'][idx]
                        current_init=100
                    elif nab>11000:
                        idx= np.random.choice(nab, size=11000, replace=False)
                        pos= g_res['absorbed_positions'][idx]
                        current_init=11000
                    else:
                        pos= g_res['absorbed_positions']
                        current_init= nab
                else:
...
def main():
    ...
    (k_sing, dk_sing)= compute_k_factor_and_uncertainty(ab_single, N0,
                                                        single_mix.fission_probability,
                                                        single_mix.u235.nu)
    ...
    def my_sim_func(fracDec, Rval, geometry, size,
        ...
        (k_, dk_)= compute_k_factor_and_uncertainty(ab_, n_neutrons,
                                                   mm.fission_probability,
                                                   mm.u235.nu)
        return (k_, dk_)
